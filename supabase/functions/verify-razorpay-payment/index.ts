...
serve(async (req) => {
  ...
  try {
    const { razorpayOrderId, razorpayPaymentId, razorpaySignature, mentorId, learnerId, slotId, message } = await req.json();
    ...
    const { data: booking, error: bookingErr } = await supabase
      .from('bookings')
      .insert({ mentor_id: mentorId, learner_id: learnerId, slot_id: slotId, message: message || null, status: 'confirmed' })
      .select()
      .single();
